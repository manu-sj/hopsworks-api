...
\t    pass"""
    ...
    def test_execute(self, mocker, execution_mode):
        ...
        if isinstance(online_test_data, pd.Series):
            assert (
                add_one.execute(args=online_test_data, online=True).values.tolist()
                == expected_online_value.values.tolist()
            )
        else:
            assert (
                add_one.execute(args=online_test_data, online=True)
                == expected_online_value
            )

        if isinstance(offline_test_data, pd.Series):
            assert (
                add_one.execute(args=offline_test_data, online=False).values.tolist()
                == offline_expected_data.values.tolist()
            )
        else:
            assert (
                add_one.execute(args=offline_test_data, online=False)
                == expected_online_value
            )
