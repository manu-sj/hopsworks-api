...
class Model(Model):
    ...
    def save(
        ...
        if not self._model and not model_path:
            ...
            _logger.info(f"Saving model located at path: `{model_path}`")
            ...
            try:
                ...
                raise ModelRegistryException(
                    "Unable to save the Python model as a pickle file. Please make sure that the model provided can be pickled or try saving the model manually."
                ) from e

        hopsworks_model = super().save(
