    ...
    def save(
        ...
        if not self._model and not model_path:
            raise ModelRegistryException(
                "Please provide the path to the saved model using the ´model_path´ argument or provide the `model` argument while creating the tensorflow model."
            )
        elif model_path:
            _logger.info(f"Saving tensorflow model found in path `{model_path}`")
        else:
            # Importing tensorflow only if required
            if not HAS_TENSORFLOW:
                raise ModuleNotFoundError(tensorflow_not_installed_message)
            else:
                import tensorflow as tf
            try:
                _logger.info("Saving tensorflow models in Keras v3.")
                model_directory = TemporaryDirectory()
                model_save_path = os.path.join(
                    model_directory.name, self.name + ".keras"
                )
                self._model.save(filepath=model_save_path)
                remove_temp_files = (
                    True if not keep_original_files else remove_temp_files
                )
            except Exception as e:
                _logger.info(
                    f"Error saving tensorflow model using Keras V3 format. {repr(e)}. Attempting to save model using savedModel format."
                )
                try:
                    model_save_path = model_directory.name
                    tf.saved_model.save(self._model, model_directory.name)
                    remove_temp_files = (
                        True if not keep_original_files else remove_temp_files
                    )
                except Exception as e:
                    raise ModelRegistryException(
                        "Unable to save the tensorflow model in `Keras V3` Format or `SavedModel` format . Please make sure that the model provided can be saved in the mentioned formats or try saving the model manually."
                    ) from e
        super().save(
        ...
        if not self._model:
            ...
            try:
                keras_model_path = os.path.join(
                    downloaded_model_directory, self.name + ".keras"
                )
                if os.path.exists(keras_model_path):
                    _logger.info("Attempting to load model from Keras V3 format.")
                    self._model = tf.keras.models.load_model(keras_model_path)
                else:
                    _logger.info("Attempting to load model using SavedModel Format.")
                    self._model = tf.saved_model.load(downloaded_model_directory)
                if not path:
